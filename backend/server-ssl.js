...
const sslOptions = {
  ...
  // ca: fs.readFileSync(path.join(process.cwd(), 'certs', 'intermediate.crt')),
};
...
// Configure Socket.IO with HTTPS and CORS
  ...
  for (let i = 0; i < retries; i++) {
    try {
      const pool = createPool({
        ...
        timezone: '+07:00',
        dateStrings: true,
        ssl: false // Disable SSL for MySQL connection if not needed
      });
...
io.on('connection', (socket) => {
  console.log('Client connected via SSL');
...
httpsServer.listen(HTTPS_PORT, '0.0.0.0', () => {
  ...
  console.log(`SSL enabled with certificates from ./certs/`);
